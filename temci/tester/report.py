...
<html>
    ...
    <body style="font-family: sans-serif;">
        ...
        """
        inner_html += self._full_single_property_comp_table()
        for prop in self.stats.properties():
            inner_html += """
                <h3>Summary regarding {prop}</h3>
            """.format(**locals())
            inner_html += self._full_single_property_comp_table(prop)
        self._write(html.format(**locals()))
    ...
    def _color_class(self, obj: BaseStatObject) -> str:
        if obj.has_errors():
            return "danger"
        if obj.has_warnings():
            return "warning"
    ...
    def _full_single_property_comp_table(self, property: str = None) -> str:
        th_cells = []
        html_app = ""
        for single in self.stats.singles:
            id, html2 = self._short_summary_modal(single)
            html_app += html2
            th_cells.append('<th><a data-toggle="modal" {{}} data-target="#{id}">{descr}</a></th>'
                            .format(id=id, descr=single.description()))
        html = """
        <table class="table">
            <thead><th>vs</th>
        """
        html += " ".join(th_cell.format("") for th_cell in th_cells)
        html += """
            </thead>
            <tbody>
        """
        for i in range(self.stats.number_of_singles()):
            html += """<tr>""" + th_cells[i].format('scope="row"')
            for j in range(self.stats.number_of_singles()):
                pair = self.stats.get_pair(i, j)
                color_class = self._color_class(pair)
                title = "Geometric mean of the mean differences relative to the means of the left"
                rel_diff = pair.rel_difference()
                obj = pair
                if property is not None:
                    color_class = self._color_class(pair[property])
                    title="Difference relative to the mean of the left"
                    rel_diff = pair[property].mean_diff_per_mean()
                    obj = pair[property]
                id, html2 = self._short_summary_modal(obj)
                html_app += html2
                html += """
                    <td class="{color_class}"><div style="width: 100%" data-content="{title}"
                        data-trigger="hover" data-toggle="popover" data-placement="auto top" data-title="Explanation">
                        <a data-toggle="modal" data-target="#{id}" style="width:100%;">{rel_diff}</a></div></td>
                """.format(**locals())
            html += """
                </tr>
            """
        ...
        """
        return html + html_app

    def _short_summary(self, obj: BaseStatObject, with_title: bool, title_level=4) -> str:
        html = ""
        if with_title:
    ...
    def _short_summary_modal(self, obj: BaseStatObject) -> t.Tuple[str, str]:
        """
              ...
              <div class="modal-dialog" role="document">
                <div class="modal-content">
                  ...
                  <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" data-dismiss="modal" onclick="window.location='#{html_id}'">More informations</button>
                  </div>
                </div>
              </div>
            </div>
        """.format(**locals())
        return modal_id, html
    ...
    def _obj_description(self, obj: BaseStatObject) -> str:
        ...
        if isinstance(obj, SingleProperty) or isinstance(obj, TestedPairProperty):
            return self._obj_description(obj.parent) + " (regarding {})".format(obj.property)
    ...
    def _format_errors_and_warnings(self, obj: BaseStatObject, show_parent: bool = True) -> str:
        ...
        html = ""
        if obj.has_errors():
            html += collapsible("Errors ({!r})".format(len(obj.errors())), obj.errors())
        if obj.has_warnings():
            html += collapsible("Warnings ({})".format(len(obj.warnings())), obj.warnings())
        return html
